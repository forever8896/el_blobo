 ...
 *
 * Smart contracts are configured in ./contract-configs.ts
 * See that file to add your custom smart contracts.
 */
...
export async function prepareAgentkitAndWalletProvider(): Promise<{
  ...
  try {
    ...
    // Initialize custom smart contract action providers
    const customContractProviders = Object.values(SMART_CONTRACT_CONFIGS).map((config) =>
      smartContractActionProvider(config)
    );
