...
import "../src/NativeRewardVault.sol";

contract NativeRewardVaultNativeTest is Test {
    NativeRewardVault internal vault;

    address internal vaultOwner;   // also owner() in NativeRewardVault
    ...
    function setUp() public {
        ...
        vault = new NativeRewardVault(vaultOwner, REG_PRICE);
    }
    ...
    function _depositFrom(address from, uint256 assets)
        internal
        returns (uint256 shares)
    {
        // Fund depositor with native token
        vm.deal(from, assets);

        vm.prank(from);
        // ✅ pass `assets` as the argument to deposit
        shares = vault.deposit{value: assets}(assets);
    }
    ...
    function testDepositMintsSharesToVaultOwnerAndUpdatesUnallocated() public {
        ...
        // Vault totalAssets should reflect the deposit (native balance)
        assertEq(vault.totalAssets(), assets);

        // Shares are minted to vaultOwner
        assertEq(vault.balanceOf(vaultOwner), shares);

        // With REG_PRICE = 1e9 and SHARE_UNIT = 1e9, assets == shares
        assertEq(shares, assets);
    ...
    function testDepositRevertsIfAssetsTooSmallForOneUnit() public {
        ...
        vm.deal(depositor, assets);

        vm.prank(depositor);
        vm.expectRevert("assets too small for 1 unit");
        // ✅ also pass `assets` here
        vault.deposit{value: assets}(assets);
    }
    ...
    function testWithdrawHappyPath() public {
        ...
        // Native underlying was sent to user
        assertEq(withdrawn, assets);
        assertEq(userBalanceAfter - userBalanceBefore, assets);

        // Vault's totalAssets should be reduced to zero
        assertEq(vault.totalAssets(), 0);
    }
}
