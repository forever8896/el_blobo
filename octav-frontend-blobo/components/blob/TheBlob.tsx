...
const ParticleLifeform = forwardRef<unknown, ParticleLifeformProps>(
  (
    ...
    const stateRef = useRef({
      targetConfig:
        STATES[initialMode as keyof typeof STATES] || STATES["idle"],
      mouseX: 0,
      mouseY: 0,
      mouseVelocity: 0,
      lastMouseX: 0,
    ...
    useImperativeHandle(ref, () => ({
      ...
      setColors: (
        c1: string | number | THREE.Color,
        c2: string | number | THREE.Color
      ) => {
      ...
      toggleStyle: (forceState?: boolean) => {
        const newState =
          forceState !== undefined ? forceState : !stateRef.current.isHighEnd;
    ...
    useEffect(() => {
      ...
      // 2. Init Camera (use container size)
      const camera = new THREE.PerspectiveCamera(60, width / height, 0.1, 100);
      camera.position.z = stateRef.current.targetZoom;
      cameraRef.current = camera;

      // 3. Init Renderer (use container size)
      const renderer = new THREE.WebGLRenderer({ alpha: true, antialias: false });
      renderer.setSize(width, height);
      renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
      renderer.toneMapping = stateRef.current.isHighEnd
      ...
      // 4. Init Post Processing (use container size)
      ...
      rgbShiftPass.uniforms["angle"].value = Math.PI * 0.25;
      ...
      const particles = new THREE.Points(
        geometry,
        stateRef.current.isHighEnd ? materialHigh : materialSimple
      );
      scene.add(particles);
      ...
      const handleMouseMove = (e: MouseEvent) => {
        if (!stateRef.current.interactionsEnabled || !containerRef.current) return;
        ...
        const dist = Math.sqrt(
          Math.pow(x - stateRef.current.lastMouseX, 2) +
            Math.pow(y - stateRef.current.lastMouseY, 2)
        );
      ...
      // Optional zoom handling if you want wheel to change zoom
      // const handleWheel = (e: WheelEvent) => {
      //   stateRef.current.targetZoom += e.deltaY * 0.005;
      //   stateRef.current.targetZoom = Math.max(
      //     2.0,
      //     Math.min(10.0, stateRef.current.targetZoom)
      //   );
      // };
      ...
      const animate = () => {
        ...
        uniforms.uSpeed.value = THREE.MathUtils.lerp(
          ...
          stateRef.current.targetConfig.speed,
          lerpFactor
        );
        uniforms.uNoiseStrength.value = THREE.MathUtils.lerp(
          uniforms.uNoiseStrength.value,
          stateRef.current.targetConfig.strength,
          lerpFactor
        );
        uniforms.uNoiseFrequency.value = THREE.MathUtils.lerp(
          uniforms.uNoiseFrequency.value,
          stateRef.current.targetConfig.freq,
          lerpFactor
        );
        uniforms.uState.value = stateRef.current.targetConfig.stateId;

        // Colors
        uniforms.uColor1.value.lerp(
          stateRef.current.targetColor1,
          colorLerpFactor
        );
        uniforms.uColor2.value.lerp(
          stateRef.current.targetColor2,
          colorLerpFactor
        );
        ...
        uniforms.uAppearance.value = THREE.MathUtils.lerp(
          uniforms.uAppearance.value,
          stateRef.current.targetAppearance,
          0.02
        );

        // Mouse & Camera
        uniforms.uMouse.value.lerp(
          new THREE.Vector2(stateRef.current.mouseX, stateRef.current.mouseY),
          0.1
        );
        uniforms.uMouseVel.value = THREE.MathUtils.lerp(
          uniforms.uMouseVel.value,
          stateRef.current.mouseVelocity,
          0.1
        );
        stateRef.current.mouseVelocity *= 0.9;
        camera.position.z = THREE.MathUtils.lerp(
          camera.position.z,
          stateRef.current.targetZoom,
          0.05
        );
        ...
        particles.rotation.y = THREE.MathUtils.lerp(
          ...
          particles.rotation.y + (targetRotY + delta * 0.05) * 0.1,
          0.04
        );
